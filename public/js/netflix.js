...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // ----------------------------------------------------------
  // 2. Ambil daftar produk Netflix dari backend, buat map: nama → id
  // ----------------------------------------------------------
  var productIdMap = {};

  try {
    var res  = await fetch(API_BASE + "/products?category=netflix");
    var data = await res.json();
    data.forEach(function (p) {
      productIdMap[p.name] = p.id;
    });
  } catch (e) {
    console.warn("Tidak bisa fetch produk Netflix dari API:", e);
  }
  ...
  // ----------------------------------------------------------
  // 5. Klik backdrop → hapus semua produk yang dipilih dari cart
  //    dan batalkan visual selected
  // ----------------------------------------------------------
  backdrop.addEventListener("click", function (e) {
    ...
    saveCart(newCart);
    updateCartUI();        // otomatis memanggil syncCardStates
