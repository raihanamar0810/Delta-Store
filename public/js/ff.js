...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // ----------------------------------------------------------
  // 2. Ambil daftar produk Free Fire dari backend, buat map: nama → id
  // ----------------------------------------------------------
  var productIdMap = {};

  try {
    var res  = await fetch(API_BASE + "/products?category=ff");
    var data = await res.json();
    data.forEach(function (p) {
      productIdMap[p.name] = p.id;
    });
  } catch (e) {
    console.warn("Tidak bisa fetch produk FF dari API:", e);
  }
