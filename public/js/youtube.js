...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  // ----------------------------------------------------------
  // 2. Ambil daftar produk Youtube dari backend, buat map: nama → id
  // ----------------------------------------------------------
  var productIdMap = {};

  try {
    var res  = await fetch(API_BASE + "/products?category=youtube");
    var data = await res.json();
    data.forEach(function (p) {
      productIdMap[p.name] = p.id;
    });
  } catch (e) {
    console.warn("Tidak bisa fetch produk Youtube dari API:", e);
  }
